...
/*    if ((outfile = _wfopen(file.get().c_str(), L"wb")) == nullptr) {
        
        return false; 
    }  
*/
     #ifdef _MSC_VER
    errno_t err = _wfopen_s(&outfile, file.wstring().c_str(), L"wb");
#else
...
bool SaveJPEG(const Path& file, const Image& image) {
    ...
    // Используем _wfopen для открытия файла в бинарном режиме
#ifdef _MSC_VER
    errno_t err = _wfopen_s(&outfile, file.wstring().c_str(), L"wb");
#else
    outfile = fopen(file.c_str(), "wb");
    ...
    while (cinfo.next_scanline < cinfo.image_height) {
        JSAMPROW row_pointer[1];
        row_pointer[0] = static_cast<JSAMPLE*>(image.GetLine(cinfo.next_scanline)); // Получаем указатель на линию
